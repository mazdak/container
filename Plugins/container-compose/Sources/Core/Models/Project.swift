...
public struct Service: Sendable {
    ...
    public let commandCleared: Bool
    public let entrypointCleared: Bool
    ...
    public let extraHosts: [ExtraHost]
    public let tty: Bool
    ...
    public init(name: String,
                 ...
                 commandCleared: Bool = false,
                 entrypointCleared: Bool = false,
        ...
        self.commandCleared = commandCleared
        self.entrypointCleared = entrypointCleared
...
public struct ExtraHost: Sendable, Equatable {
    public let hostname: String
    public let address: String

    public init(hostname: String, address: String) {
...
public struct HealthCheck: Sendable {
    ...
    var dependencyWaitTimeoutSeconds: Int {
        let steadyInterval = max(interval ?? 5.0, 0.1)
        let startupInterval = max(startInterval ?? steadyInterval, 0.1)
        let allowedRetries = max(1, retries ?? 10)
        let startupGrace = max(startPeriod ?? 0, 0)
        let upperBound = startupGrace + startupInterval + Double(max(allowedRetries - 1, 0)) * steadyInterval
        return max(120, Int(ceil(upperBound)))
    }
}
