...
public struct ComposeFile: Codable {
    ...
    public let include: [ComposeInclude]?
...
public struct ComposeService: Codable {
    ...
    public let extraHosts: ExtraHosts?
    public let tty: Bool?
    public let stdinOpen: Bool?
    public var envFileEnvironment: [String: String] = [:]
    ...
    public init(
        ...
        command: StringOrList? = nil,
        entrypoint: StringOrList? = nil,
        workingDir: String? = nil,
        environment: Environment? = nil,
        envFile: StringOrList? = nil,
        ...
        self.envFileEnvironment = envFileEnvironment
    }
}
...
public struct ComposeInclude: Codable, Equatable {
    public let path: StringOrList
    ...
    public init(path: StringOrList, projectDirectory: String? = nil, envFile: StringOrList? = nil) {
        self.path = path
    ...
    public init(from decoder: Decoder) throws {
        if let singleValue = try? decoder.singleValueContainer(),
           let path = try? singleValue.decode(StringOrList.self) {
            self.init(path: path)
            return
        }
        ...
        self.init(
            path: try container.decode(StringOrList.self, forKey: .path),
            projectDirectory: try container.decodeIfPresent(String.self, forKey: .projectDirectory),
            envFile: try container.decodeIfPresent(StringOrList.self, forKey: .envFile)
        )
    }

    public func encode(to encoder: Encoder) throws {
        if projectDirectory == nil && envFile == nil {
...
public enum ExtraHosts: Codable {
    case list([String])
    case dict([String: String])
        ...
        case .list(let list):
            ...
            for item in list {
                if let equals = item.firstIndex(of: "=") {
                    let key = String(item[..<equals])
                    let value = String(item[item.index(after: equals)...])
                    dict[key] = value
                } else if let colon = item.firstIndex(of: ":") {
                    let key = String(item[..<colon])
