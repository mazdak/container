...
public struct DependencyResolver {
    ...
    /// Resolve dependencies only within the provided service selection.
    ///
    /// This is used by lifecycle operations like `start`, `stop`, and `restart`,
    /// which should preserve dependency ordering among the requested services
    /// without implicitly adding or requiring services outside that selection.
    public static func resolveWithinSelection(services: [String: Service]) throws -> ResolutionResult {
        try resolve(services: scopeToSelection(services: services))
    }
    ...
    static func scopeToSelection(services: [String: Service]) -> [String: Service] {
        let selectedNames = Set(services.keys)
        return services.mapValues { service in
            Service(
                name: service.name,
                image: service.image,
                build: service.build,
                ...
                dependsOnCompletedSuccessfully: service.dependsOnCompletedSuccessfully.filter(selectedNames.contains),
