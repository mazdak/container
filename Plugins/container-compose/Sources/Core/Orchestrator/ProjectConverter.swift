...
public struct ProjectConverter {
    ...
    public init(log: Logger, projectDirectory: URL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)) {
        self.log = log
    ...
    public func convert(
        ...
        let filteredServices: [String: ComposeService]
        if selectedServices.isEmpty {
            filteredServices = filterServicesByProfiles(resolvedServices, profiles: profiles)
        } else {
            filteredServices = filterSelectedServices(resolvedServices, selected: selectedServices, profiles: profiles)
        }
    ...
    private func mergeServices(base: ComposeService, override: ComposeService) throws -> ComposeService {
        ...
        return ComposeService(
            ...
            extraHosts: mergeExtraHosts(base: base.extraHosts, override: override.extraHosts),
            tty: override.tty ?? base.tty,
            stdinOpen: override.stdinOpen ?? base.stdinOpen,
            envFileEnvironment: mergeEnvFileEnvironment(base: base.envFileEnvironment, override: override.envFileEnvironment)
        )
    }

    private func mergeEnvFileEnvironment(base: [String: String], override: [String: String]) -> [String: String] {
    ...
    private func mergeExtraHosts(base: ExtraHosts?, override: ExtraHosts?) -> ExtraHosts? {
        guard let base = base else { return override }
        guard let override = override else { return base }
    ...
    private func convertService(name: String, service: ComposeService, projectName: String) throws -> Service {
        ...
        let volumes: [VolumeMount] = (service.volumes ?? []).compactMap { spec -> VolumeMount? in
            ...
            case .object(let obj):
                ...
                default: // bind
                    let src0 = (obj.source ?? "")
                    return normalizeBindPathIfNeeded(
                        VolumeMount(
                            source: src0,
                            target: obj.target,
                            readOnly: ro,
                            type: .bind,
                            createHostPath: obj.bind?.createHostPath ?? true
                        )
                    )
                }
            }
        ...
        let healthCheck: HealthCheck? = {
            ...
            if !testCommand.isEmpty {
                if testCommand[0] == "NONE" {
                    ...
                    return nil
                } else if testCommand[0] == "CMD" && testCommand.count > 1 {
                    testCommand = Array(testCommand.dropFirst())
            ...
            return HealthCheck(
                ...
                startInterval: parseTimeInterval(hc.startInterval)
            )
        }()
        ...
        let parsedCommand = try parseCommandArguments(service.command)
        let parsedEntrypoint = try parseCommandArguments(service.entrypoint)

        return Service(
            name: name,
            image: service.image,
            build: normalizeBuildConfig(service.build),
            command: parsedCommand.arguments,
            entrypoint: parsedEntrypoint.arguments,
            commandCleared: parsedCommand.cleared,
            entrypointCleared: parsedEntrypoint.cleared,
            ...
            extraHosts: convertExtraHosts(service.extraHosts),
            cpus: cpus,
            memory: memory,
    ...
    private func normalizeBindPathIfNeeded(_ mount: VolumeMount) -> VolumeMount {
        ...
        if !src.hasPrefix("/") {
            src = projectDirectory.appendingPathComponent(src).standardized.path
        }
        // Resolve existing symlinks so host sockets like /var/run/docker.sock mount their real source.
        if FileManager.default.fileExists(atPath: src) {
            src = URL(fileURLWithPath: src).resolvingSymlinksInPath().path
        }
        return VolumeMount(
            source: src,
            target: mount.target,
            ...
            type: mount.type,
            createHostPath: mount.createHostPath
        )
    }

    private func normalizeBuildConfig(_ build: BuildConfig?) -> BuildConfig? {
        guard let build else { return nil }

        let resolvedContextURL = resolveProjectPath(build.context ?? ".")
        let resolvedDockerfileURL: URL = {
            guard let dockerfile = build.dockerfile, !dockerfile.isEmpty else {
                return resolvedContextURL.appendingPathComponent("Dockerfile")
            }
            if dockerfile.hasPrefix("/") {
                return URL(fileURLWithPath: dockerfile).standardizedFileURL
            }
            return resolvedContextURL.appendingPathComponent(dockerfile).standardizedFileURL
        }()

        return BuildConfig(
            context: resolvedContextURL.path,
            dockerfile: resolvedDockerfileURL.path,
            args: build.args,
            target: build.target
        )
    }

    private func resolveProjectPath(_ rawPath: String) -> URL {
        var path = rawPath
        if path.hasPrefix("~") {
        ...
        return projectDirectory.appendingPathComponent(path).standardizedFileURL
    }
    ...
    private func parseCommandArguments(_ value: StringOrList?) throws -> ParsedCommandArguments {
        guard let value else {
            return ParsedCommandArguments(arguments: nil, cleared: false)
        }

        switch value {
        case .list(let list):
            if list.isEmpty {
                return ParsedCommandArguments(arguments: [], cleared: true)
            }
            return ParsedCommandArguments(arguments: list, cleared: false)
        case .string(let string):
            if string.isEmpty {
                return ParsedCommandArguments(arguments: [], cleared: true)
            }
            return ParsedCommandArguments(arguments: try ComposeShellWords.split(string), cleared: false)
        }
    }
    ...
    private func convertExtraHosts(_ extraHosts: ExtraHosts?) -> [ExtraHost] {
        guard let extraHosts else { return [] }
        return extraHosts.asDictionary.map { ExtraHost(hostname: $0.key, address: $0.value) }
    }
    ...
    private func parseTimeInterval(_ string: String?) -> TimeInterval? {
        ...
        for match in matches {
            ...
            consumed.append(String(string[range]))

            switch String(string[unitRange]) {
            case "ns":
        ...
        return consumed == string ? total : nil
    }

    private func loadEnvFile(url: URL, expansionEnvironment: [String: String]) throws -> [String: String] {
        ...
        for rawLine in text.split(whereSeparator: { $0.isNewline }) {
            ...
            val = expandVariables(in: val, existingVars: out, expansionEnvironment: expansionEnvironment)
    ...
    private func expandVariables(in value: String, existingVars: [String: String], expansionEnvironment: [String: String]) -> String {
        do {
            return try ComposeVariableInterpolator.interpolate(value) { variable in
                if let existingValue = existingVars[variable] {
            ...
            log.warning("Failed to expand env_file value '\(value)': \(error)")
            return value
        }
    }
}
