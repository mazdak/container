...
- **Compose Parity Additions**:
  ...
  - Long-form volumes: `type` bind/volume/tmpfs, `~` expansion, project-relative path normalization, supports `ro|rw|z|Z|cached|delegated`
  ...
  - `tty` respected for container init processes
  - Image pulling policy on `compose up --pull` with `always|missing|never`
  - Health gating with `service_started`, `service_healthy`, and `--wait/--wait-timeout`
  - Top-level Compose `name:` used as the default project name when `-p/--project` is not set
...
- `--wait`: block until services reach running/healthy state. This implies detached behavior.
...
- The parser loads variables from `.env` in the directory of the first compose file for interpolation.
- Precedence: shell environment overrides `.env` values. Variables already set in the environment are not overwritten by `.env`.
- You can also pass variables explicitly with `--set-env KEY=VALUE` (repeatable).
...
- `depends_on.condition=service_completed_successfully` is currently unsupported because the runtime API does not expose container exit status.
- Recreation flags: `--force-recreate` and `--no-recreate` respected; config hash drives default reuse behavior.
- `stdin_open`, `configs`, `secrets`, and several advanced Compose-spec keys are not implemented yet.
